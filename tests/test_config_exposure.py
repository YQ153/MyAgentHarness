...
WHY 需要它：服务默认绑 ``127.0.0.1``，此时只有本机能访问；一旦改绑 ``0.0.0.0``，
任何能访问该地址的人都能直接驱动 ``execute`` 工具。这个状态不报错、不崩溃、
不影响任何功能，只会安静地存在——所以只能靠显式断言把它钉住。

覆盖两层，缺一层就存在绕过路径：
1. 判定函数 ``is_loopback_host``——误报会训练用户忽略告警，漏报则等于没有检查；
2. 两条入口的接线（``main._run_web`` / ``main._run_cli``）——防止「检查写了但没
...
def test_loopback_bind_stays_quiet(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    config = make_config(tmp_path, host="127.0.0.1")
    caplog.clear()

    with caplog.at_level(logging.ERROR):
        warned = config.warn_if_publicly_exposed()

    assert warned is False
    assert caplog.records == []


def test_exposed_bind_warns(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    config = make_config(tmp_path, host="0.0.0.0")
    caplog.clear()
    ...
    assert "对外暴露风险" in record.getMessage()
...
def test_effective_host_overrides_config(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.ERROR):
        warned = config.warn_if_publicly_exposed("0.0.0.0")
...
def test_non_string_effective_host_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="effective_host"):
        config.warn_if_publicly_exposed(8000)  # type: ignore[arg-type]
