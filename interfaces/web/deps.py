...
#: 会掉进 500，用户看到一个与他的操作毫无关系的服务端故障。``SessionPresetLockedError``
#: （场景与已锁定的不符）同理：它是"这条会话已经属于另一个场景"，重试无用，只能新建会话。
...
async def resolve_scoped_services(
    ...
    *,
    ...
    Args:
        ...
        preset: 请求里给出的场景预设 ID；``None`` 表示不指定。草稿态下用它预览该场景的
            技能集；已锁定的会话以**库里的场景**为准（不一致会被拒成 409）。

    Returns:
        该会话生效的会话根服务集合。

    Raises:
        HTTPException: 400（路径不存在或不是目录）、404（会话不存在）、409（根或场景已
            锁定、还没确定，或不可用）。
    """
    registry = get_session_registry(request)
    try:
        return await registry.services_for(
            requested=requested, thread_id=thread_id, allow_missing=True, preset=preset
        )
