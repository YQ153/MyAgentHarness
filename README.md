...
- **应用数据集中在一个 `.harness/` 目录里**：技能库、技能视图、工具输出留存与知识库索引都住在
  工作区内的 `<工作区>/.harness/{skills,skills-active,tool-outputs,knowledge.db}`（跟着项目走，
  换机器、换工作空间之后行为一致）。它们对 Agent 只经固定虚拟路径暴露（`/skills`、
  `/.skills-active`、`/_tool_outputs`），且**只读**——包括 `/.harness/` 本身那条兜底路由。
  技能包请放进 `<工作区>/.harness/skills/`。**文件面板不会列出 `.harness/`**；若希望它在版本
  控制里被忽略，自己往 `.gitignore` 写一行 `.harness/`（应用不替用户改仓库里的文件）。
- **升级说明**：该工作空间第一次被使用时，两代旧位置里的技能包与留存会**自动搬进** `.harness/`：
  ① `<数据目录>/roots/<根标识>/`（2026-09-21～09-22 的布局）；② 更早的 `<工作区>/skills/`
...
- **隔离**：命名空间按主体收敛（`("memories", <user_id>)`）。当前仓库没有认证子系统，
  主体恒为匿名，实际命名空间是 `("memories", "__anonymous__")`——面板与 Agent 读的是同一份。
  要按人分开，得先有主体标识的来源（`agent/run_context.py`），那一层目前是常量 `local`。
...
├── skills/                   # 随应用交付的技能资产
│   ├── builtin/              #   通用技能（每个场景都能用；挂虚拟路径后由 Agent 读取）
│   └── presets/              #   场景预设：每个子目录一个场景（preset.toml + 可选技能包）
