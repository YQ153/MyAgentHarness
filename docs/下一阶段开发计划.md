...
- [ ] **待补（Sprint 1 收尾）**：审计事件目前只删不导出；若需离线留存，
      应在 `purge_expired` 处挂归档 sink，而不是让保留策略依赖人工导出

> **T3 期间发现的既有问题（未修，不阻断）：** 本机 `uv run lint-imports`
> 以 GBK 解码 `.importlinter`（含中文注释）失败，分层契约无法在 Windows
> 中文环境自动校验；本次新增导入已人工核对满足六条契约
> （`interfaces` 不直连 `runtime`、`runtime` 为叶子层等）。
> 建议后续把该命令纳入 UTF-8 环境（如 CI 或 `PYTHONUTF8` 生效的终端）执行。
