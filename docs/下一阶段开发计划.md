...
### Sprint 4（草案，待确认）：能力边界扩展

> **选单依据：** Sprint 1~3 已清零 P0 与「可部署/可量化」类 P1。剩余 P1 中，
> 按「决定能力天花板 + 工程量可控」排序，建议取前三项开工；范围与顺序
> 待用户确认后定稿。

#### T8 MCP / 自定义工具注册（2~3 天）

- [ ] 配置化 MCP server（`MCP_SERVERS` 走 `config.py`，不硬编码）
- [ ] `agent/tools.py`：自定义工具注册器，与 deepagents 内置工具并存，
      冲突时显式报错而非静默覆盖
- [ ] 工具调用统一进审计（`tool_call` 事件已有，需补 server/工具名与耗时）
- [ ] 补测试：注册冲突、调用失败降级、审计字段完整

#### T9 长期记忆持久化 + 记忆管理（2~3 天）

- [ ] 用 LangGraph `BaseStore` 的 SQLite 实现替换 `runtime/store.py` 的
      `InMemoryStore`（当前「跨会话记忆」重启即丢）
- [ ] `GET/DELETE /api/memories`：按 owner 收敛，member 只看自己的
- [ ] 前端记忆面板：查看 / 删除单条记忆

#### T10 会话管理：重命名 / 搜索 / 归档（1~2 天）

- [ ] `PATCH /api/threads/{id}` 重命名、`GET /api/threads?query=` 搜索
- [ ] 归档（软删除）与列表过滤；删除会话时同步清理 HITL 挂起与用量归属
