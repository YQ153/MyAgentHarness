...
> - **仍待办（属仓库设置，代码侧无法完成）**：开启主干分支的
>   "Require status checks to pass" 并选中 `test / ubuntu-latest` 与 `test / windows-latest`；
>   在那之前 CI 只是提示，不构成拦阻。
...
- [ ] 主干分支（当前为 `main`）开启合并前必须通过该流水线（**属仓库设置，待人工开启**）
...
- [ ] **修 D-4**：新增「工具执行中段取消」用例——脚本化图 + 同步阻塞工具，断言取消后
      `DONE(reason=stopped)` 收尾、槽位释放、`abort_scope` 被调用、进程树无残留；
      把 `scripts/smoke_stop_kill.py` 的真机结论固化进测试套件
- [ ] **修 D-5**：新增 `scripts/smoke_providers.py`——按注册表逐个 provider 构造并做最小调用；
      未配置密钥或地址的 provider **跳过并打印原因**；退出码区分「全通过 / 有跳过 / 有失败」
- [ ] **修 D-6**：确认无调用后删除 `interfaces/web/auth/device_flow.py::_device_flow_expires_at`
- [ ] **修 D-7**：评估 `normalize_thread_id` 的真实归属——若判定为领域规则，下沉到与
      `text_utils.py` 同级的中立模块并删除 `application/thread_id.py` 门面；
      若维持现状，在文件 docstring 写明「门面为长期形态」及理由（**不留悬而未决的过渡态**）
- [ ] **修 D-9**：同步《分层架构评估与重构方案》10.2 节目标态依赖图与实际结构
      （`bootstrap` 由 `interfaces` 依赖）
- [ ] 验收：`pytest` 全绿且总数 > 605；`scripts/smoke_providers.py` 在仅配 DeepSeek 的机器上
      输出「deepseek 通过 / 其余跳过（未配置）」；全仓检索 `_device_flow_expires_at` 命中 0
