...
> 它记录的是**方案定型时**的设计与取舍，其中的代码片段（如 `build_store` /
> `InMemoryStore`）在后来的重构中已被替换——照抄会拿到已经不存在的东西。
> 当时的结论已全部落地（分层契约 6 条 KEPT，由 CI 每次运行校验），
> **现状请以代码、`README.md` 与《功能完整度分析与缺失功能清单.md》为准。**
> 本文档保留原样，是为了不丢失「当时为什么这么选」这段信息：改写它，
> 等于把决策依据也一并改掉。
...
> **2026-09-19 附注**：本节是 2026-09-16 的**现状快照**，保留用于复核当时的判断依据
> （其中的 ⚠ 标记即修复对象），**不是当前结构**。期间已发生的变化：
> 阶段一删除了 `tool/` 与 `test.txt`；`rate_limiter.py` 已下沉到 `runtime/`；
> `interfaces/web/auth.py` 已拆为 `interfaces/web/auth/` 子包；`application/thread_id.py`
> 门面已删除（规则下沉为根级 `thread_utils.py`）；`application/run_service.py` 已按职责
> 拆为 `run_service` / `run_registry` / `run_governance` / `run_branch` 四块。
> **当前结构请以 `docs/overview/architecture.html` 第 2、3 章为准**；历次结构变更见
> 《分层架构重构实施记录》与《运行服务模块拆分实施记录》。
