...
WHY 需要这个脚本，而不是走 REST：``GET /api/threads/{id}/export`` 确实能回答
「调了哪些工具」，但它要求服务在运行，而且拿不到 ``invalid_tool_calls``、
``reasoning_content`` 这类字段——判定「模型只是在思考里提到了某个工具名」还是
「真的发起了调用」恰好依赖这些字段。排障时最常见的三种处境（服务起不来、
会话已被真删除、库正在被写）REST 都覆盖不到。
